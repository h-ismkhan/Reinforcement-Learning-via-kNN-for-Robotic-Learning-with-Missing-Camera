...
epoch_study.py
==============
Epoch-count ablation for RL4IL on a single LIBERO suite.

For each value in EPOCH_SET, ALL training phases are run with that
epoch count:
  • PPO_EPOCHS      = E   (prediction RL policy)
  • FUSION_EPOCHS   = E   (prediction FusionHead)
  • IMP_PPO_EPOCHS  = E   (imputation RL policy, per modality)
  • IMP_SOFT_EPOCHS = E   (soft imputation head, per modality)

Results are written to a CSV:
  <suite>_<variant>_<strategy>_sf<0|1>_epochs.csv
  Columns: epochs, success_rate_mean, success_rate_std

Only the DROPOUT configs are evaluated (mask_0 and mask_1) and their
mean is reported as the single success-rate number, since that is the
paper's main claim. Change STUDY_CONFIGS below to include "none" if needed.
...
LIBERO_SUITE = "libero_goal"   # change to libero_object / libero_goal / libero_spatial

DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")

# ── Epoch study ───────────────────────────────────────────────────────────────
EPOCH_SET = [1, 5, 10, 15, 20]   # each value = unified epoch count for all phases

# Which dropout configs to include in the reported success rate
# "none"   = all cameras present
# "mask_0" = agent cam missing
# "mask_1" = in-hand cam missing
STUDY_CONFIGS = ["mask_0", "mask_1"]   # paper's main claim
...
# ── RL hyperparams (fixed, only epoch count varies) ───────────────────────────
PPO_CLIP       = 0.2
LR             = 3e-4
ENT_COEF       = 0.0001
MINIBATCH_SIZE = 128
...
USE_SOFT_FUSION = True   # set False for hard-argmax ablation
...
def build_all_sets(emb,labels,adj):
    ...
    print(f"  Sets built: {sum(s is not None for s in all_sets)}/{N} | mean size: {v.mean():.1f}")
...
class FusionHead(nn.Module):
    def __init__(self,emb_dim,hidden=FUSION_HIDDEN,n_heads=FUSION_HEADS,topk=FUSION_TOPK):
        super().__init__()
...
# =============================================================================
# TRAINING FUNCTIONS  (epoch count passed as argument)
# =============================================================================
...
def _infer_one_fusion(q_vec,knn_idxs,knn_dists,tr_emb,tr_labels,adj,tr_set_sizes,policy,head):
...
def train_policy(tr_emb,tr_labels,tr_sets,tr_oracles,
                 va_emb,va_labels,adj,set_sizes,n_epochs):
    D=tr_emb.shape[1]
...
def train_fusion(policy,tr_emb,tr_labels,tr_sets,va_emb,va_labels,adj,set_sizes,n_epochs):
    if not USE_SOFT_FUSION: return None
    D=tr_emb.shape[1]; head=FusionHead(emb_dim=D).to(DEVICE)
        ...
        with torch.no_grad():
            for qi in range(len(va_labels)):
                _,s=_infer_one_fusion(va_emb[qi],ki[qi],kd[qi],
                                       tr_emb,tr_labels,adj,set_sizes,policy,head)
                preds.append(s)
    ...
    for epoch in range(n_epochs):
        ...
        perm=rng.permutation(len(valid)); ep_loss,nb=0.0,0
        for start in range(0,len(valid),FUSION_BATCH):
            mb=[valid[perm[j]] for j in range(start,min(start+FUSION_BATCH,len(valid)))]
            bl=[]
            for i in mb:
                ce,cl,_=_topk_from_sequence(tr_emb[i],tr_sets[i],tr_emb,tr_labels,policy,FUSION_TOPK)
...
def train_imp_policy_one(mod_idx,tr_raw,va_raw,stats,n_epochs):
...
def train_soft_imp_head_one(mod_idx,imp_policy,imp_index,valid_emb_m,adj_imp,
    ...
    for epoch in range(n_epochs):
        ...
        perm=rng.permutation(len(train_items)); ep_loss,nb=0.0,0
        for start in range(0,len(train_items),IMP_SOFT_BATCH):
            ...
            for i in mb:
                ...
                state,cfs=_imp_features(q_partial,gt_emb_m,cset,valid_emb_m)
                with torch.no_grad():
                    sc=imp_policy(torch.tensor(state,device=DEVICE),torch.tensor(cfs,device=DEVICE))
...
def impute_one_sample(raw_mods_i,present_mask,stats,imp_policies,soft_heads):
...
def build_imputed_emb(raw_list,present_mask_all,stats,imp_policies,soft_heads):
    ...
    for task_id in range(task_suite.n_tasks):
        ...
        for seed_idx in range(N_ROLLOUT_SEEDS):
            ...
            for rollout_i,qi_local in enumerate(qidxs):
                qi,_=tlist[qi_local]
                best_tr_idx,_=_infer_one_fusion(te_emb_cfg[qi],ki_all[qi],kd_all[qi],
...
# =============================================================================
# ONE FULL RUN FOR A GIVEN EPOCH COUNT
# =============================================================================

def run_one_epoch_count(E, train_demos, val_demos, test_demos, task_suite,
                        tr_emb, tr_labels, va_emb, va_labels,
                        te_emb, te_labels, tr_raw, va_raw, te_raw, stats):
    """
    Train everything from scratch with E epochs for every phase.
    Returns mean success rate across STUDY_CONFIGS.
    """
    print(f"\n{'#'*60}")
    ...
    # Phase 1: prediction RL + fusion
    print(f"\n[E={E}] Training prediction RL …")
    policy = train_policy(tr_emb, tr_labels, all_sets, all_oracles,
                          va_emb, va_labels, adj, set_sizes, n_epochs=E)
    print(f"[E={E}] Training fusion head …")
    ...
    for mod_idx in range(N_MODS):
        if mod_idx == 2:   # language never imputed
            imp_policies.append(None); soft_heads.append(None); continue
        print(f"[E={E}] Imputation RL mod={MOD_NAMES[mod_idx]} …")
        result = train_imp_policy_one(mod_idx, tr_raw, va_raw, stats, n_epochs=E)
        imp_policies.append(result)
        pm, imp_index_m, valid_idx_m, valid_emb_m, adj_imp_m = result
        print(f"[E={E}] Soft imputation head mod={MOD_NAMES[mod_idx]} …")
        sh = train_soft_imp_head_one(mod_idx, pm, imp_index_m, valid_emb_m,
                                     adj_imp_m, tr_raw, va_raw, stats, n_epochs=E)
        soft_heads.append(sh)

    # Phase 3: rollout evaluation for STUDY_CONFIGS
    config_srs = []
    for config_label, cam0_present, cam1_present in EVAL_CONFIGS:
        if config_label not in STUDY_CONFIGS:
            continue
        present_mask = np.array([cam0_present, cam1_present, True], dtype=bool)
        print(f"\n[E={E}] Building imputed embeddings [{config_label}] …")
        tr_emb_cfg = build_imputed_emb(tr_raw, present_mask, stats, imp_policies, soft_heads)
        te_emb_cfg = build_imputed_emb(te_raw, present_mask, stats, imp_policies, soft_heads)
        adj_cfg = _build_knn_graph(tr_emb_cfg)
        sets_cfg, oracles_cfg, sizes_cfg = build_all_sets(tr_emb_cfg, tr_labels, adj_cfg)
        print(f"[E={E}] Retraining prediction policy on imputed space …")
        policy_cfg = train_policy(tr_emb_cfg, tr_labels, sets_cfg, oracles_cfg,
                                  va_emb, va_labels, adj_cfg, sizes_cfg, n_epochs=E)
        head_cfg   = train_fusion(policy_cfg, tr_emb_cfg, tr_labels, sets_cfg,
                                  va_emb, va_labels, adj_cfg, sizes_cfg, n_epochs=E)
        print(f"[E={E}] Evaluating rollouts [{config_label}] …")
        ...
        config_srs.append(mean_sr)

    overall_mean = float(np.mean(config_srs)) if config_srs else 0.0
    print(f"\n[E={E}] Overall mean SR across {STUDY_CONFIGS}: {overall_mean:.3f}")
    return overall_mean
...
def main():
    import copy   # needed inside training functions
    globals()['copy'] = copy
    ...
    print(f"Epoch set: {EPOCH_SET}")
    print(f"Study configs: {STUDY_CONFIGS}\n")
    ...
    cfg_tag  = "-".join(STUDY_CONFIGS)
    csv_path = f"{LIBERO_SUITE}_{VARIANT}_{STRATEGY}_{sf_tag}_{cfg_tag}_epochs.csv"

    # ── epoch study loop ──────────────────────────────────────────────────────
    rows = []
    for E in EPOCH_SET:
        mean_sr = run_one_epoch_count(
            E, train_demos, val_demos, test_demos, task_suite,
            tr_emb, tr_labels, va_emb, va_labels,
            te_emb, te_labels, tr_raw, va_raw, te_raw, stats)
        rows.append((E, mean_sr))
        # save after each epoch count so partial results are not lost
        with open(csv_path, "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerow(["epochs", "success_rate_mean"])
            writer.writerows(rows)
        print(f"\n  CSV updated → {csv_path}\n")

    # ── final summary ─────────────────────────────────────────────────────────
    print("\n" + "="*50)
    print(f"  Epoch study complete — {LIBERO_SUITE}")
    print(f"  {'Epochs':<10s}  {'Success Rate':>14s}")
    print(f"  {'-'*10}  {'-'*14}")
    for E, sr in rows:
        print(f"  {E:<10d}  {sr:.3f}")
    print("="*50)
