...
LIBERO_SUITE = "libero_object"           # <-- changed from libero_spatial
...
ENT_COEF       = 0.0001
MINIBATCH_SIZE = 128
# suite-specific checkpoint names so object runs don't overwrite spatial ones
RL_CKPT        = f"best_rl_{LIBERO_SUITE}_{VARIANT}_{STRATEGY}.pt"
...
FUSION_LR      = 3e-4
FUSION_BATCH   = 32
FUSION_CKPT    = f"best_fusion_{LIBERO_SUITE}_{VARIANT}_{STRATEGY}.pt"
...
IMP_LR              = 3e-4
IMP_ENT_COEF        = 0.0001
IMP_CKPT_PREFIX     = f"best_imp_{LIBERO_SUITE}_{VARIANT}_{STRATEGY}"
...
IMP_SOFT_LR         = 3e-4
IMP_SOFT_BATCH      = 32
IMP_SOFT_CKPT_PREFIX = f"best_soft_imp_{LIBERO_SUITE}_{VARIANT}_{STRATEGY}"

# ── Skip training and load saved checkpoints ──────────────────────────────────
# Set True to jump straight to Phase 3 if checkpoints already exist
SKIP_TRAINING = False

# ── Ablation: False = hard RL argmax, True = soft fusion head (full RL4IL)
...
def main():
    ...
    # DisDP reference numbers for libero_object (from Table 1 and Table 4)
    disdp_ref = {
        "libero_object": {
            "none":   "0.816 ± 0.02",
            "mask_0": "0.295 ± 0.04",
    ...
    out_json = f"results_libero_spatial_{VARIANT}_{STRATEGY}.json"
